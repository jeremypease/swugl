...
def send_verification_email(user, url):
    return send_email(
        user.email,
        "Verify your Swugl email address",
        f"""
        <h2>Welcome to Swugl, {user.first_name}!</h2>
        <p>Please verify your email address by clicking the link below:</p>
        <p><a href="{url}">Verify Email Address</a></p>
        <p>This link will expire in 24 hours.</p>
        <p>If you did not register for Swugl, please ignore this email.</p>
        """
    )
...
def send_approval_notification(user, url):
    return send_email(
        user.email,
        "Your Swugl account has been approved!",
        f"""
        <h2>Welcome to Swugl, {user.first_name}!</h2>
        <p>Your account has been approved. You can now log in at:</p>
        <p><a href="{url}">Sign In to Swugl</a></p>
        """
    )
...
def send_pending_notification(admin_email, new_user, url):
    return send_email(
        admin_email,
        "New Swugl registration pending approval",
        f"""
...
def send_spouse_confirmation_email(requesting_person, target_user, confirm_url, decline_url):
    return send_email(
        target_user.email,
        "Spouse connection request on Swugl",
        f"""
        <h2>Spouse Connection Request</h2>
        <p>{requesting_person.get_display_name()} has indicated that you are their spouse on Swugl.</p>
...
def send_password_reset_email(user, reset_url):
    return send_email(
        user.email,
        "Reset your Swugl password",
        f"""
        <h2>Password Reset</h2>
        <p>Hi {user.first_name},</p>
        <p>We received a request to reset your Swugl password. Click the link below to set a new one:</p>
...
def send_member_invitation_email(inviting_name, person_first_name, family_name, to_email, url):
    return send_email(
        to_email,
        f"You've been invited to join {family_name} on Swugl",
        f"""
        <h2>You're invited to Swugl!</h2>
        <p>Hi {person_first_name},</p>
        <p>{inviting_name} has added you to the {family_name} family on Swugl
...
def send_spouse_invitation_email(inviting_person, to_email, url):
    return send_email(
        to_email,
        "You've been invited to join Swugl",
        f"""
        <h2>You're Invited to Swugl!</h2>
        <p>{inviting_person.get_display_name()} has invited you to join Swugl,
...
def send_trial_warning_email(admin, family, days_left, billing_url):
    return send_email(
        admin.email,
        f"Your Swugl trial ends in {days_left} day{'s' if days_left != 1 else ''}",
        f"""
        <h2>Hi {admin.first_name},</h2>
...
def send_pod_added_email(user, family_name, dashboard_url):
    return send_email(
        user.email,
        f"You've been added to {family_name} on Swugl",
        f"""
